...
if ( ! class_exists( 'Jet_Engine_Listings_Macros' ) ) {
	...
	class Jet_Engine_Listings_Macros {
		...
		public function get_all( $sorted = false, $escape = false ) {
			...
			if ( null === $this->macros_list ) {
				...
				$macros_list = apply_filters( 'jet-engine/listings/macros-list', array(
					...
					'object_id' => array(
						'label' => esc_html__( 'Object ID (for current context)', 'jet-engine' ),
						'cb'    => array( $this, 'get_object_id' ),
					),
					'current_tags' => array(
					...
					'current_meta' => array(
						'label' => esc_html__( 'Current meta value', 'jet-engine' ),
					...
					'current_meta_string' => array(
						'label' => esc_html__( 'Current meta value as string', 'jet-engine' ),
					...
					'related_parents_from' => array(
						'label' => esc_html__( 'Related parents from (legacy)', 'jet-engine' ),
					...
					'related_children_from' => array(
						'label' => esc_html__( 'Related children from (legacy)', 'jet-engine' ),
					...
					'related_children_between' => array(
						'label' => esc_html__( 'Related children between (legacy)', 'jet-engine' ),
		 ...
		 * @return [type]              [description]
		 */
		public function field_name( $field_value = null, $args = '' ) {

			$args       = explode( '|', $args );
			$field_name = $args[0];
			$return     = ! empty( $args[1] ) ? $args[1] : 'field_name';
