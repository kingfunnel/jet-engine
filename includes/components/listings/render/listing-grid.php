...
if ( ! class_exists( 'Jet_Engine_Render_Listing_Grid' ) ) {
	...
	class Jet_Engine_Render_Listing_Grid extends Jet_Engine_Render_Base {
		...
		public function print_no_listing_notice() {
			...
			if ( ! empty( $_GET['context'] ) && 'edit' === $_GET['context'] && ! empty( $_GET['attributes'] ) ) {
				printf( '<div class="jet-listing-notice">%1$s</div>', $notice );
			} else {
				printf(
					'<div class="jet-listing-notice">%1$s <a href="%3$s" onClick="( function( e ){ window.open( e.target.href, \'_blank\' ).focus(); })( arguments[0] );" target="_blank">%2$s</a></div>',
					$notice,
					__( 'Or create a new one', 'jet-engine' ),
					esc_url( admin_url( 'edit.php?post_type=jet-engine#add_new' ) )
				);
			}
